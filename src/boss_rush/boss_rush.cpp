...
bool boss_rush_is_fighting_here() {
    ...
    if (!g_configBossRushSeparateGanon && std::strcmp(target.displayName, "Ganondorf") == 0) {
        if (is_boss_rush_ganon_stage(curStage)) {
            return true;
        }
        return false;
    }
...
static void update_beastganon_instant_fight(bool gameFrameTick) {
    ...
    if (t < 0 || static_cast<size_t>(t) >= g_bossGalleryCount ||
        std::strcmp(g_bossGalleryTable[t].displayName, "Beast Ganon") != 0) {
        return;
    }
...
static void update_ganon_ground_duel() {
    ...
    if (std::strcmp(g_bossGalleryTable[t].displayName, "Ganondorf") != 0) return;
    ...
    if (!s_duelReady) {
        ...
        if (bbi::ganondorf_horse_demo_pending(gnd)) {
...
static void commit_boss_rush_fight_warp(size_t i, daAlink_c* link,
    ...
    if (isGanonGauntlet) {
        ...
        dComIfGp_setNextStage("D_MN09A", 0, 50, 0, 0.0f, 0, 1, 0, 0, 0, 0);
        return;
    }
