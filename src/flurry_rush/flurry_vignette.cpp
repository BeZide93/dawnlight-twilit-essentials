...
float effect_level_now() {
    const double now = now_seconds();
    float dt = 0.0f;
    if (s_lastFrameSeconds > 0.0) {
        dt = static_cast<float>(std::min(now - s_lastFrameSeconds, 0.1));
    }
...
ModResult build_pipeline(ModError* error) {
    ...
    moduleDesc.label = {"flurry vignette", WGPU_STRLEN};
    WGPUShaderModule module = wgpuDeviceCreateShaderModule(s_device.device, &moduleDesc);
    if (module == nullptr) {
        return mods::set_error(error, MOD_ERROR, "failed to compile flurry_vignette.wgsl");
    }
    ...
    desc.label = {"flurry vignette", WGPU_STRLEN};
    ...
    if (s_pipeline == nullptr) {
        return mods::set_error(error, MOD_ERROR, "failed to create flurry vignette pipeline");
    }
...
ModResult init_flurry_vignette(const GfxService* gfx_svc, const ResourceService* res_svc,
    ...
    ModResult result = s_res->load(s_ctx, "flurry_vignette.wgsl", &s_shaderSource);
    if (result != MOD_OK || s_shaderSource.data == nullptr) {
        return mods::set_error(error, result != MOD_OK ? result : MOD_ERROR,
            "failed to load flurry_vignette.wgsl");
    }

    if (s_gfx->get_device_info(s_ctx, &s_device) != MOD_OK) {
        return mods::set_error(error, MOD_ERROR, "flurry vignette: failed to query device info");
    }
    if (s_gfx->get_scene_target_layout(s_ctx, &s_sceneLayout) != MOD_OK) {
        return mods::set_error(error, MOD_ERROR, "flurry vignette: failed to query scene layout");
    }
    ...
    samplerDesc.label = {"Flurry vignette linear sampler", WGPU_STRLEN};
    samplerDesc.magFilter = WGPUFilterMode_Linear;
    samplerDesc.minFilter = WGPUFilterMode_Linear;
    s_sampler = wgpuDeviceCreateSampler(s_device.device, &samplerDesc);
    if (s_sampler == nullptr) {
        return mods::set_error(error, MOD_ERROR, "failed to create flurry vignette sampler");
    }

    GfxDrawTypeDesc drawDesc = GFX_DRAW_TYPE_DESC_INIT;
    drawDesc.label = "flurry vignette";
    drawDesc.draw = on_draw;
    if (s_gfx->register_draw_type(s_ctx, &drawDesc, &s_drawType) != MOD_OK) {
        return mods::set_error(error, MOD_ERROR, "flurry vignette: failed to register draw type");
    }
    ...
    {
        return mods::set_error(error, MOD_ERROR, "flurry vignette: failed to register stage hook");
    }

    return MOD_OK;
}

void update_flurry_vignette(const LogService*, ModContext*) {
}
