// Damage vignette - structured 1:1 after the dusklight-shaders composite
// (fs_grade): fullscreen triangle, scene color snapshot + uniform + sampler
// in bind group 0, RGB-only overwrite so scene alpha survives. The only
// functional difference to the shader pack's grade vignette: instead of
// darkening the edges, they are mixed toward red by the pushed intensity.
...
fn fs_main(in: VSOut) -> @location(0) vec4<f32> {
    ...
    let red = vec3<f32>(0.62, 0.04, 0.02);
    let out = mix(c, red, clamp(mask, 0.0, 1.0));
